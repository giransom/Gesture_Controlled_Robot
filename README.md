...
In the completion of this project there were many challenges to overcome. The first challenge faced was in connecting the bluetooth modules together. I based my original wiring based on the original project I found my HC 05 modules would not enter AT mode and respond to commands. When I further researched the modules independently I found the EN pin needed to be fed 3.3 V when entering AT mode. My next challenge faced was ensuring the actual modules not only were connected by checking the blinking rate, but sending actual characters between each other. My solution found was making sure the correct baud rate was set (38400 baud) in AT mode and then setting the Arduino's Serial bauds to be different values (Uno @ 115200 and Nano BLE 33 @ 9600). My next challenge faced was providing enough voltage to the motor driver to run while disconnected from the computer. The solution to this issue was to wire a separate 9V battery to the motor driver so it had a consistent source of voltage. After finishing the base model of the project I decided to add an additional HPV camera with touch screen display. In adding my addition components I ran into many issues, starting with issues of my camera connecting to the wifi. Originally when I setup my camera the streaming worked, but after restarting the camera it suddenly did not connect to wifi anymore. Eventually I found that I forgot to add an additional antenna the camera came with which prevented the board from reaching the wifi. After that initial issue it was simple to start to set up the display screen and ensure it could pick up the streamed IP website. When my display screen finally started to stream the video feed there was issues with setting ip the actual pixel setting since the JPEG decoder was not adjusted correctly.
